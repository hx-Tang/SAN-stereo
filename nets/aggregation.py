...
from nets.gaa import GAABottleneck, GAABottleneck0
        ...
        for i in range(self.num_scales):
            ...
            for j in range(num_blocks):
                if simple_bottleneck:
                    ...
                    branch.append(GAABottleneck0(num_candidates, num_candidates // 16, num_candidates))
                else:
...
# Guided & self attention aggregation
class GAAModule(nn.Module):
    def __init__(self, num_scales, num_output_branches, max_disp,
                 num_blocks=1,
                 kernel_size = 7):
        super(GAAModule, self).__init__()
        ...
        for i in range(self.num_scales):
            ...
            branch = nn.ModuleList()
            for j in range(num_blocks):
                    branch.append(GAABottleneck(1, num_candidates, num_candidates//16, num_candidates, self.kernel_size))
...
# Stacked GAAModules
class GAAggregation(nn.Module):
    def __init__(self, max_disp, num_scales=3, num_fusions=6,
                 num_stage_blocks=1,
                 intermediate_supervision=True):
        super(GAAggregation, self).__init__()
        ...
        for i in range(num_fusions):
            ...
            fusions.append(GAAModule(num_scales=self.num_scales,
                                                     num_output_branches=num_out_branches,
                                                     max_disp=max_disp,
                                                     num_blocks=num_stage_blocks))
    ...
    def forward(self, cost_volume, leftfeature):
        assert isinstance(cost_volume, list)
        assert isinstance(leftfeature, list)

        for i in range(self.num_fusions):
            fusion = self.fusions[i]
            cost_volume = fusion(cost_volume, leftfeature)
